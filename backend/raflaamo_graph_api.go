...
// TODO: This is the function where problems arise.
// TODO: make the goroutine channel thing work correctly.
func interact_with_api(all_possible_time_slots []covered_times, id_from_reservation_page_url string, current_date string, amount_of_eaters int) ([]parsed_graph_data, error) {
	data := make([]parsed_graph_data, 0, len(all_possible_time_slots))
	response_chan := make(chan *http.Response, len(all_possible_time_slots))
	wp := workerpool.New(2)
	...
	for response := range response_chan {
		deserialized_graph_data, err := deserialize_graph_response(&response)
		...
		data = append(data, *deserialized_graph_data)
	}
...
func get_available_time_intervals_from_graph_api(id_from_reservation_page_url string, time_slots_to_check_from_graph_api []covered_times, amount_of_eaters int, all_reservation_times []int64, current_time date_and_time) ([]string, error) {
	...
	for _, api_response := range api_responses {
